                            ...
                            <select name="section_id" class="w-full p-4 pl-12 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none text-sm font-bold appearance-none cursor-pointer text-slate-700">
                                ...
                                @foreach($sections as $sec)
                                    <option value="{{ $sec->id }}" {{ $selectedSection && $selectedSection->id == $sec->id ? 'selected' : '' }}>
                                        {{-- 
                                            LOGIC: 
                                            1. Show Grade Level & Section Name
                                            2. Check if Section is linked to a School Year. If yes, display it.
                                            3. Check if Teacher is the Adviser.
                                        --}}
                                        Grade {{ $sec->grade_level }} - {{ $sec->name }} 
                                        
                                        @if($sec->schoolYear)
                                            (SY {{ $sec->schoolYear->school_year }})
                                        @endif

                                        {{ $sec->teacher_id == $currentTeacherId ? ' ★ Advisory' : '' }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="px-10 py-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <h3 class="font-black text-2xl text-slate-900 tracking-tight">
                        @if($selectedSection)
                            ...
                            {{-- Show School Year in Title as well --}}
                            @if($selectedSection->schoolYear)
                                <span class="text-slate-400 text-lg font-bold ml-2">(SY {{ $selectedSection->schoolYear->school_year }})</span>
                            @endif
                                ...
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <img src="{{ 
                                             ...
                                             class="w-12 h-12 rounded-[1.25rem] border-2 border-white shadow-md group-hover:scale-110 transition-transform duration-500 object-cover bg-slate-100">
                ...
                @else
                    <div class="py-32 text-center relative overflow-hidden group">
                        ...
                        <div class="relative z-10">
                            ...
                            <p class="text-slate-400 max-w-sm mx-auto font-medium leading-relaxed px-6">Select one of your handled sections from the filter above to access the official student master list.</p>
                        </div>
                    </div>
                @endif
            </div>
