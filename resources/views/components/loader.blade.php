<!-- Enhanced FMNHS Preloader -->
<div id="global-loader" class="fixed inset-0 z-[100] flex items-center justify-center transition-all duration-500" style="will-change: opacity, transform;">
    <!-- Animated gradient background -->
    <div class="absolute inset-0 loader-bg"></div>
    ...
    <!-- Main content -->
    <div class="relative flex flex-col items-center gap-5 loader-entrance">
        <!-- School logo with glow ring -->
        <div class="relative">
            <div class="loader-glow-ring"></div>
            <div class="w-20 h-20 rounded-full overflow-hidden border-2 border-white/30 shadow-2xl relative z-10 bg-white/10 backdrop-blur-sm">
                <img src="{{ asset('images/fmnhs.png') }}" alt="FMNHS" class="w-full h-full object-cover" loading="eager">
            </div>
        </div>
    ...
    >
      ...
      <stop stop-opacity="0" stop-color="white" offset="1"></stop>
        ...
        <div class="text-center">
            <h2 class="text-white font-bold text-lg tracking-wider loader-title">FMNHS</h2>
            <p class="text-white/50 text-[10px] font-medium tracking-[0.3em] uppercase mt-0.5">Student Information System</p>
        </div>

        <!-- Progress bar -->
        <div class="w-48 loader-progress-container">
            <div class="h-1 w-full rounded-full bg-white/10 overflow-hidden backdrop-blur-sm">
                <div id="loader-progress-bar" class="h-full rounded-full loader-progress-fill" style="width: 0%"></div>
            </div>
            <p id="loader-status-text" class="text-white/40 text-[10px] text-center mt-2 font-medium tracking-widest uppercase loader-status">
...
<style>
    /* === Background === */
    .loader-bg {
        background: linear-gradient(135deg, #0f0a1e 0%, #1a1145 30%, #0d1b2a 60%, #0a0f1e 100%);
    }
    ...
    .loader-orb-1 {
        ...
        background: radial-gradient(circle, #5727CC 0%, transparent 70%);
        top: -100px; right: -50px;
        animation: orbFloat1 8s ease-in-out infinite;
    }
    .loader-orb-2 {
        width: 250px; height: 250px;
        background: radial-gradient(circle, #91DDFB 0%, transparent 70%);
        bottom: -80px; left: -60px;
        animation: orbFloat2 10s ease-in-out infinite;
    }
    .loader-orb-3 {
        width: 200px; height: 200px;
        background: radial-gradient(circle, #4354BF 0%, transparent 70%);
        top: 40%; left: 60%;
    ...
    .loader-glow-ring {
        ...
        border-radius: 50%;
        z-index: 5;
        background: conic-gradient(from 0deg, #5727CC, #91DDFB, #4354BF, #97E6FF, #5727CC);
    ...
    /* === Entrance animation === */
    .loader-entrance {
        animation: loaderSlideIn 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }
    ...
    /* === Title shimmer === */
    .loader-title {
        background: linear-gradient(90deg, #fff 0%, #91DDFB 50%, #fff 100%);
    ...
    .loader-progress-fill {
        background: linear-gradient(90deg, #5727CC, #91DDFB, #97E6FF);
        background-size: 200% 100%;
        animation: progressShimmer 2s ease-in-out infinite;
        transition: width 0.4s cubic-bezier(0.22, 1, 0.36, 1);
        box-shadow: 0 0 12px rgba(145, 221, 251, 0.5);
    }
    ...
    /* === SVG wrapper === */
    .loader-svg-wrapper {
        filter: drop-shadow(0 0 20px rgba(87, 39, 204, 0.3));
    }
    ...
    /* === Progress animation on re-show (navigation) === */
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const statusMessages = ['Initializing', 'Loading assets', 'Preparing', 'Almost ready'];

    // Simulate progress on initial load
    function animateProgress(targetWidth, duration, onComplete) {
        const start = parseFloat(progressBar.style.width) || 0;
        const increment = (targetWidth - start) / (duration / 16);
        let current = start;
    ...
    // Staged progress: 0->30 fast, 30->70 medium, 70->100 on ready
    animateProgress(30, 200, function() {
        if (statusText) statusText.innerHTML = statusMessages[1] + '<span class="loader-dots"></span>';
        animateProgress(65, 300, function() {
            if (statusText) statusText.innerHTML = statusMessages[2] + '<span class="loader-dots"></span>';
            animateProgress(90, 200, function() {
                if (statusText) statusText.innerHTML = statusMessages[3] + '<span class="loader-dots"></span>';
                // Final burst to 100 then hide
                setTimeout(function() {
                    animateProgress(100, 100, function() {
                        hideLoader();
                    });
                }, 150);
            });
    ...
    function showLoader() {
        ...
        if (statusText) statusText.innerHTML = 'Navigating<span class="loader-dots"></span>';

        loader.classList.remove('hidden', 'loader-hiding');
        loader.style.opacity = '1';
        loader.style.transform = '';

        // Animate progress for navigation
        setTimeout(function() {
            animateProgress(40, 300, function() {
                animateProgress(70, 600, function() {
                    // Will complete when page loads
                });
            });
        }, 50);
    }
    ...
    const links = document.querySelectorAll(
        '.nav-link, a[data-show-loader="true"], a[href]:not([href^="#"]):not([href^="javascript:"]):not([href^="mailto:"]):not([href^="tel:"])'
    );
    ...
    document.querySelectorAll('form:not([data-no-loader])').forEach(function(form) {
        form.addEventListener('submit', function() {
            showLoader();
            if (statusText) statusText.innerHTML = 'Processing<span class="loader-dots"></span>';
        });
    });
